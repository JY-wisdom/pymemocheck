...
class drawfigure(object):
    ...
    def procdata(self):
        ...
        try:
            ...
            if len(slist)%3 != 0:
                messagebox.showerror('输入区间信息错误')
                return
            for line in slist:
                line = re.match('[A-Za-z0-9]{1,20}', line)
                line = line.group(0)
                if index == 0:
                    key = line
                    index = 1
        ...
        for key in self.eeprom1.keys():
            ...
            for i in range(self.eeprom1[key][0], self.eeprom1[key][1]):
                if valuey != self.array1y[int(i)]:
                    ...
                    self.ax1.annotate(stri,xy=(i, valuey), xytext=(i, valuey), xycoords='data')
